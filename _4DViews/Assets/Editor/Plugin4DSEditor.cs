...
namespace unity4dv
{
    ...
    public class Plugin4DSEditor : Editor
    {
        ...
        private void BuildFilesInspector(Plugin4DS myTarget)
        {
            ...
            myTarget.playAudio = EditorGUILayout.Toggle(new GUIContent("Play Audio", "Will play the audio included in 4ds if any is present"), myTarget.playAudio);
            ...
            var list = myTarget.ListEvents;
            list.show = EditorGUILayout.Toggle("Show events", list.show);
            if (list.show) {
                for(int i = 0; i < list.eventsFrames.Count; i++) {
                    EditorGUILayout.TextField("Frame : " + list.eventsFrames[i], list.eventsNames[i]);
                }
            }
